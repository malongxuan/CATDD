...
class Translator(object):
    ...
    def _run_encoder(self, batch, data_type):
        ...
        his_memory_bank, last_memory_bank, knl_memory_bank = self.model.encoder(src, knl)
        if src_lengths is None:
            assert not isinstance(last_memory_bank, tuple), \
                'Ensemble decoding only supported for text data'
            src_lengths = torch.Tensor(batch.batch_size) \
                               .type_as(last_memory_bank) \
                               .long() \
                               .fill_(last_memory_bank.size(0))
        return src, knl, his_memory_bank, last_memory_bank, knl_memory_bank, src_lengths
    ...
    def _fast_translate_batch(
        ...
        # Encoder forward.
        src, knl, enc_states, src_memory_bank, knl_memory_bank, src_lengths = self._run_encoder(
            batch, data.data_type)
        self.model.decoder.init_state(src, knl, src_memory_bank, enc_states)
        ...
        if isinstance(src_memory_bank, tuple):
            src_memory_bank = tuple(tile(x, beam_size, dim=1) for x in src_memory_bank)
        ...
        if isinstance(knl_memory_bank, tuple):
            knl_memory_bank = tuple(tile(x, beam_size, dim=1) for x in knl_memory_bank)
        ...
        for step in range(max_length):
            ...
            # Reorder states.
            if isinstance(src_memory_bank, tuple):
                src_memory_bank = tuple(x.index_select(1, select_indices)
                                    for x in src_memory_bank)
            else:
                src_memory_bank = src_memory_bank.index_select(1, select_indices)
    ...
    def _translate_batch(self, batch, data):
        ...
        src, knl, his_memory_bank, last_memory_bank, knl_memory_bank, src_lengths = self._run_encoder(batch, data_type)
        his, last = src[:100, :, :], src[100:, :, :]
        self.model.decoder.init_state(last, last, None, None)

        first_dec_words = torch.zeros((self.max_length, batch_size, 1)).fill_(self.model.encoder.embeddings.word_padding_idx).long()
        ...
        if "tgt" in batch.__dict__:
            results["gold_score"] = self._score_target(
                batch, his_memory_bank, last_memory_bank, knl_memory_bank, knl
            )
            self.model.decoder.init_state(last, last, None, None)
        else:
            results["gold_score"] = [0] * batch_size
        ...
        if isinstance(his_memory_bank, tuple):
            his_memory_bank = tuple(tile(x, beam_size, dim=1) for x in his_memory_bank)
            mb_device = his_memory_bank[0].device
        else:
            his_memory_bank = tile(his_memory_bank, beam_size, dim=1)
            mb_device = his_memory_bank.device

        if isinstance(last_memory_bank, tuple):
            last_memory_bank = tuple(tile(x, beam_size, dim=1) for x in last_memory_bank)
            mb_device = last_memory_bank[0].device
        else:
            last_memory_bank = tile(last_memory_bank, beam_size, dim=1)
            mb_device = last_memory_bank.device
        ...
        for i in range(self.max_length):
            ...
            dec_out, dec_attn = self.model.decoder(
                inp, last_memory_bank, his_memory_bank, memory_lengths=memory_lengths, step=i
            )
        ...
        self.model.decoder2.init_state(first_dec_words[:50, :, :], knl, None, None)
        first_dec_words = self.model.encoder.embeddings(first_dec_words).transpose(0, 1).contiguous()
        decode1_memory_bank = self.model.encoder.last_transformer(first_dec_words[:, :50, :], None).transpose(0, 1).contiguous()
        if isinstance(decode1_memory_bank, tuple):
            decode1_memory_bank = tuple(tile(x, beam_size, dim=1) for x in decode1_memory_bank)
    ...
    def _score_target(self, batch, his_memory_bank, last_memory_bank, knl_memory_bank, knl):
        tgt_in = inputters.make_features(batch, 'tgt')[:-1]

        first_dec_out, first_attns = self.model.decoder(tgt_in, last_memory_bank, his_memory_bank, memory_lengths=None)
        ...
        self.model.decoder2.init_state(first_dec_words, knl, None, None)
        first_dec_words = self.model.encoder.embeddings(first_dec_words).transpose(0, 1).contiguous()
        decode1_bank = self.model.encoder.last_transformer(first_dec_words, None).transpose(0, 1).contiguous()
