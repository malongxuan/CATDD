...
from onmt.inputters.ntext_dataset import TextDataset
...
def load_fields_from_vocab(vocab, data_type="text"):
    ...
    n_knl_features = len(collect_features(vocab, 'knl'))
    fields = get_fields(data_type, n_src_features, n_tgt_features, n_knl_features)
...
def filter_example(ex, use_src_len=True, use_tgt_len=True, use_knl_len=True,
                   min_src_len=1, max_src_len=float('inf'),
                   min_tgt_len=1, max_tgt_len=float('inf'),
                   min_knl_len=1, max_knl_len=float('inf')):
    """
    ...
    return (not use_src_len or min_src_len <= len(ex.src) <= max_src_len) and \
        (not use_tgt_len or min_tgt_len <= len(ex.tgt) <= max_tgt_len) and \
        (not use_knl_len or min_knl_len <= len(ex.knl) <= max_knl_len)
...
def build_vocab(train_dataset_files, fields, data_type, share_vocab,
                knl_vocab_path, knl_vocab_size, knl_words_min_frequency,
    ...
    # Load vocabulary
    if knl_vocab_path:
        knl_vocab = load_vocabulary(knl_vocab_path, "knl")
        knl_vocab_size = len(knl_vocab)
        logger.info('Loaded source vocab has %d tokens.' % knl_vocab_size)
        for i, token in enumerate(knl_vocab):
            # keep the order of tokens specified in the vocab file by
            # adding them to the counter with decreasing counting values
            counters['knl'][token] = knl_vocab_size - i
    else:
        knl_vocab = None
    ...
    if data_type == 'text':
        ...
        # knowledge
        _build_field_vocab(
            fields["knl"], counters["knl"],
            max_size=knl_vocab_size, min_freq=knl_words_min_frequency)
        logger.info(" * knl vocab size: %d." % len(fields["knl"].vocab))

        # All datasets have same num of n_src_features,
        # getting the last one is OK.
        n_knl_feats = sum('knl_feat_' in k for k in fields)
        for j in range(n_knl_feats):
            key = "knl_feat_" + str(j)
            _build_field_vocab(fields[key], counters[key])
            logger.info(" * %s vocab size: %d." %
                        (key, len(fields[key].vocab)))
        ...
        if share_vocab:
            ...
            _merge_field_vocabs(
                fields["knl"], fields["src"], fields["tgt"], vocab_size=knl_vocab_size,
...
def load_fields(dataset, opt, checkpoint):
    ...
    if data_type == 'text':
        logger.info(' * vocabulary size. source = %d; target = %d; knowledge = %d' %
                    (len(fields['src'].vocab), len(fields['tgt'].vocab), len(fields['knl'].vocab)))
    else:
...
def _collect_report_features(fields):
    knl_features = collect_features(fields, side='knl')
    src_features = collect_features(fields, side='src')
    tgt_features = collect_features(fields, side='tgt')

    return knl_features, src_features, tgt_features
